...
module.exports = {
	...
	start: async function ({ api, globalGoat, args, message, event, download }) {
		...
		let imgBuffer;
		try {
			imgBuffer = (await axios.get(`https://api-ntk-production.up.railway.app/taoanhdep/batslap?apikey=ntkhang&author=${linkAvatar(senderID)}&target=${linkAvatar(targetId)}`, {
